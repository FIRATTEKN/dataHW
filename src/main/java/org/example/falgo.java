...
public class falgo {
    ...
    public int MSE (double[] exSmoothing,double[] doubleExSmoothing,double[] regressionAn,double[] deseasonalizedRegAn){
        double sumMseEx = 0;
        double sumMseDEx = 0;
        double sumMseReg = 0;
        ...
        for (i =0; i < exSmoothing.length; i++) {
            sumMseEx += (x[i]-exSmoothing[i])*(x[i]-exSmoothing[i]);
            sumMseDEx += (x[i]-doubleExSmoothing[i])*(x[i]-doubleExSmoothing[i]);
            sumMseReg += (x[i]-regressionAn[i])*(x[i]-regressionAn[i]);
            sumMseDeReg += (x[i]-deseasonalizedRegAn[i])*(x[i]-deseasonalizedRegAn[i]);
        }
